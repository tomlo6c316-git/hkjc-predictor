...
# 載入模型的函數 (假設模型檔與 app.py 放在同一個資料夾或雲端)
@st.cache_resource
def load_model():
                ...
                for table in tables:
                    for row in table.find_all('tr'):
                        ...
                        if len(cols) >= 3 and cols[0] in ['獨贏', 'Win']:
                            try:
                                odds_dict[cols[1]] = float(cols[2].replace(',', ''))
                            except:
                                continue
...
if run_button:
    if model is None:
        st.error("⚠️ 找不到模型檔案 `my_hkjc_model.pkl`，請確保它與此 App 放在同一資料夾中！")
    else:
        ...
        if result_df is not None and not result_df.empty:
            ...
            for race_id, group in result_df.groupby('賽事編號'):
                ...
                with st.expander(f"📍 {race_name} (賽事編號: {race_id})"):
                    ...
                    if not filtered.empty:
                        ...
                        display_cols = ['馬號', '馬名', '騎師', '練馬師', '獨贏賠率', 'pred_win_prob', 'ev']
                        ...
                        st.warning("此場賽事暫無符合您設定門檻的高 EV 馬匹（寧缺勿濫）。")
            ...
            st.error("⚠️ 無法獲取該日期的賽事資料，請確認日期是否正確或當日是否有賽事。")
